...
class Model_kasir extends CI_Model {
		...
		function ambil_tmp_kasir()
		{
			...
			$this->db->join('tbl_harga_jual','tmp_kasir.id_produk=tbl_harga_jual.id_produk','left');
			$this->db->join('tbl_harga_beli','tmp_kasir.id_produk=tbl_harga_beli.id_produk','left');
		...
		function struk($id_transaksi)
		{
			$this->db->select('
			mst_transaksi.seq,
			mst_transaksi.id_transaksi,
			mst_transaksi.id_member,
			mst_transaksi.id_produk,
			mst_transaksi.id_terapis,
			mst_transaksi.harga_beli,
			mst_transaksi.harga_jual,
			mst_transaksi.disc,
			mst_produk.nama,
			mst_produk.deskripsi,
			tbl_terapis.nama AS terapis,
			tbl_terapis.id_terapis,
			');
			$this->db->where('mst_transaksi.id_transaksi',$id_transaksi);
			$this->db->from('mst_transaksi');
			$this->db->join('mst_produk','mst_transaksi.id_produk=mst_produk.id_produk','left');
			$this->db->join('tbl_terapis','mst_transaksi.id_terapis=tbl_terapis.id_terapis','left');
			return $this->db->get();  
		}
